      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {files.images.map((img) => (
            <a key={img.id} href={fileUrl(img.storagePath)} target="_blank" rel="noopener" className="block">
              <img src={fileUrl(img.storagePath)} alt={img.filename} className="h-24 w-full object-cover rounded border border-gray-200" />
            </a>
          ))}
        </div>
        {canEdit && (
        ...
        <ul className="space-y-1 text-sm">
          {files.documents.map((doc) => (
            <li key={doc.id} className="flex items-center justify-between">
              <a href={fileUrl(doc.storagePath)} target="_blank" rel="noopener" className="text-brand-700 hover:underline truncate">
                {doc.filename}
              </a>
